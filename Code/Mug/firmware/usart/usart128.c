			     ...
			     uint8_t flags)
{
	...
	/*Set baud rate */
	//ubrr = USART_UBRR_VAL(baudrate) - 2;
...
void usart_send_bytes(struct usart_dev *dev, char *bytes, uint16_t len)
{
	uint16_t i;
	for (i=0; i<len; i++)
...
void usart_send_string(struct usart_dev *dev, char *bytes)
{
	char c;
	while ((c = *bytes++))
